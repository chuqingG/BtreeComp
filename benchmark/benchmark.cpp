...
enum class BenchmarkTypes { INSERT, SEARCH, RANGE, BACKWARDSCAN };

enum class DatasetTypes { RANDOM, ELSE }; // need to add further
...
void PerformanceBenchmarkResults(
  ...
  for (auto benchmark : benchmarks) {
    ...
    if (write_to_file) {
      if (benchmark == BenchmarkTypes::RANGE) {
        myfile << "\n"
               << benchmarkStrMap.at(benchmark)
               << " performance: " << float(1 * RANGE_SCOPE) << "\n"
...
void TreeStatisticBenchmarkResults(
  ...
  if (write_to_file) {
    string file_name = output_path + ".txt";
    myfile.open(file_name, fstream::out | ios::app);
    myfile << "\nname\theight\tkeysize\tprefixsize\tbranchdegree\tnodes\tnonlea"
              "fnodes\n";
  }
...
void RunBenchmark() {
  ...
  if (dataset == DatasetTypes::RANDOM) {
    column_num = 1;
    generate_random_number(values, key_numbers * 0.8);
    generate_random_number(values_warmup, key_numbers * 0.2);
  } else {
    ...
    if (key_numbers) {
      ...
      max_data_length = read_dataset_char(allvalues, dataset_file_name,
                                          max_keylen, key_numbers);
    } else {
      max_data_length =
          read_dataset_char(allvalues, dataset_file_name, max_keylen);
...
int main(int argc, char *argv[]) {
  ...
  if (output_file_arg != nullptr) {
    ...
    if (output_path_str == "auto") {
      output_path = generate_output_path(datasets_arg, key_numbers, iterations,
                                         0, max_keylen);
    } else {
