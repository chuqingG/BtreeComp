...
int BPTreeMT::searchRange(string min,string max){
    vector<Node *> parents;
    Node *leaf = search_leaf_node(root, min, parents);
...
int BPTreeMT::backwardScan(string min, string max) {
    vector<Node *> parents;
    Node *leaf = search_leaf_node(root, max, parents);
    Node *prevleaf = nullptr;
    if(leaf != nullptr && this->head_comp)
...
vector<string> BPTreeMT::decompress_keys(Node *node, int pos) {
