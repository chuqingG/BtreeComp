...
int find_insert_pos(NodeDB2 *node, string &key, int (*insertfunc)(NodeDB2 *, string_view, int, int, bool &), bool &equal) {
    ...
    if (metadatapos == node->prefixMetadata.size()) {
        ...
        if (key.compare(0, prefix.length(), prefix) == 0) {
            ...
            insertpos = insertfunc(node, key, metadata.low, metadata.high, equal);
            if (!equal)
...
db2split perform_split(NodeDB2 *node, int split, bool isleaf) {
    ...
    for (uint32_t i = 0; i < segment.size(); i++) {
        ...
        for (int j = low; j <= high; j++) {
            ...
            if (newprefix.compare("") == 0) {
                keys.at(j).value = decompressed;
            }
            else {
                keys.at(j).value = decompressed.substr(newprefix.length());
            }
        }
    }
    ...
    if (prefixlen > 0 && prefixlen >= max(p_i.prefix.length(), p_i_plus_1.prefix.length())) {
        for (int i = p_i.low; i <= p_i.high; i++) {
            ...
            if (decompressed.compare(0, prefixlen, common_prefix) == 0) {
                ...
                if (firstlow == -1)
                    firstlow = i;
                keys.at(i).value = decompressed.substr(prefixlen);
            }
        }
...
void apply_prefix_optimization(NodeDB2 *node) {
    if (node->prefixMetadata.size() == 1) {
        ...
        for (uint32_t i = 1; i < node->keys.size(); i++) {
            ...
            if (uninitialized) {
                Key key = node->keys.at(i - 1);
                key.value = prevkey.substr(currprefix.length());
                newkeys.push_back(key);
                key = node->keys.at(i);
                key.value = currkey.substr(currprefix.length());
                newkeys.push_back(key);
                ...
                Key key = node->keys.at(i);
                key.value = currkey.substr(prevprefix.length());
                newkeys.push_back(key);
                prefixmetadata.high += 1;
            }
            else {
        ...
        if (uninitialized) {
            Key key = node->keys.at(node->keys.size() - 1);
            key.value = prevkey;
            newkeys.push_back(key);
