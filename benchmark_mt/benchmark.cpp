...
auto RunBenchmarkIteration(std::vector<char*> values, std::vector<char*> values_warmup){
    ...
    auto time_gap = static_cast<double>(std::chrono::duration_cast<
                    std::chrono::nanoseconds>(std::chrono::system_clock::now() - t_start).count()) / 1e9;
            ...
            switch (benchmark) {
                ...
                case BenchmarkTypes::SEARCH:
                    ...
                    if(structure->Search(values))
                        cout << name << "\t:" << "No errors happen during search" << endl;
                    else
                        cout << name << "\t:" << "Errors happen during search" << endl;
                    time_spent = static_cast<double>(std::chrono::duration_cast<
                    std::chrono::nanoseconds>(std::chrono::system_clock::now() - t1).count()) / 1e9;
                    break;
#ifndef CHARALL
...
void RunBenchmark(){
    ...
    if(dataset == DatasetTypes::RANDOM){
        ...
        // cout << "max_size():" << values.max_size() << endl;
	    // cout << "size():" << values[0].size() << endl;
	    // cout << "capacity():" << values[0].capacity() << endl;
        sleep(10);
    }
    ...
    const auto time = static_cast<double>(std::chrono::duration_cast<
        std::chrono::seconds>(after_bench - t1).count()) / 60;
