...
private:
	...
	static int insert_binary(DB2Node *cursor, string_view key, int low, int high);
	int search_binary(DB2Node *cursor, string_view key, int low, int high);
	DB2Node* search_leaf_node(DB2Node *root, string_view key, vector<DB2Node *> &parents);
	...
	DB2Node* scan_node(DB2Node *node, string key);
