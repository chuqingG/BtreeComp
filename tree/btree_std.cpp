...
BPTree::BPTree(bool head_compression,
...
int BPTree::search(const char *key) {
    vector<Node *> parents;
    Node *leaf = search_leaf_node(_root, key, parents);
    if (leaf == nullptr)
        return -1;
    if (this->head_comp) {
        return search_binary(leaf, key + leaf->prefix.size(), 0, leaf->size - 1);
    }
    else {
        return search_binary(leaf, key, 0, leaf->size - 1);
    }
...
#ifdef TIME_DEBUG
void BPTree::insert_time(char *x, double &prefix_calc, double &prefix_update, double &search, double &insert) {
    if (__root == nullptr) {
        __root = new Node;
#ifdef DUPKEY
        int rid = rand();
    ...
    Node *leaf = search_leaf_node(_root, x, parents);
    TIMECOUNT(insert, insert_leaf_time, leaf, parents, x, prefix_calc, prefix_update);
...
int BPTree::searchRange(string min, string max) {
    ...
    int pos = leaf != nullptr ? search_binary(leaf, min, 0, leaf->size - 1) : -1;
    int entries = 0;
...
int BPTree::backwardScan(string min, string max) {
    ...
    int pos = leaf != nullptr ? search_binary(leaf, max, 0, leaf->size - 1) : -1;
    int entries = 0;
...
void BPTree::insert_nonleaf(Node *node, vector<Node *> &parents, int parentlevel, splitReturn childsplit) {
    if (check_split_condition(node, childsplit.promotekey.data())) {
        Node *parent = nullptr;
        if (parentlevel >= 0) {
            parent = parents.at(parentlevel);
        }
        splitReturn currsplit = split_nonleaf(node, parents, parentlevel, childsplit);
        if (node == _root) {
...
#endif
            InsertKey(newRoot, 0, currsplit.promotekey.data());
        ...
        char *newkey = childsplit.promotekey.data();
        int insertpos;
        bool equal = false;
        if (this->head_comp) {
            // promotekey = promotekey.substr(node->prefix.length());
            insertpos = insert_binary(node, newkey + node->prefix.length(), 0, node->size - 1, equal);
        }
        else {
            insertpos = insert_binary(node, newkey, 0, node->size - 1, equal);
        }
#ifdef DUPKEY
        int rid = rand();
        ...
        if (this->head_comp) {
            char *key_comp = newkey + node->prefix.length();
...
void BPTree::insert_leaf(Node *leaf, vector<Node *> &parents, char *key) {
    ...
    if (check_split_condition(leaf, key)) {
        ...
        if (leaf == _root) {
            ...
            // TODO: to fix
#endif
            InsertKey(newRoot, 0, split.promotekey.data());
    ...
    // Don't need to split
        ...
        bool equal = false;
        if (this->head_comp) {
            insertpos = insert_binary(leaf, key + leaf->prefix.length(), 0, leaf->size - 1, equal);
        }
        else {
            insertpos = insert_binary(leaf, key, 0, leaf->size - 1, equal);
        }
#ifdef DUPKEY
        vector<Key_c> allkeys;
        int rid = rand();
        ...
        if (!equal) {
            if (this->head_comp) {
                char *key_comp = key + leaf->prefix.length();
...
#ifdef TIME_DEBUG
void BPTree::insert_leaf_time(Node *leaf, vector<Node *> &parents, char *key, double &prefix_calc, double &prefix_update) {
    if (check_split_condition(leaf)) {
        splitReturn split = split_leaf_time(leaf, parents, key, prefix_calc, prefix_update);
        if (leaf == _root) {
            Node *newRoot = new Node;
            int rid = rand();
#ifdef DUPKEY
...
int BPTree::split_point(Node *node) {
    ...
    if (this->tail_comp) {
        ...
        for (int i = split_range_low; i < split_range_high - 1; i++) {
            if (i + 1 < size) {
                string compressed = tail_compress(GetKey(node, i),
                                                  GetKey(node, i + 1));
...
splitReturn BPTree::split_nonleaf(Node *node, vector<Node *> parents, int pos, splitReturn childsplit) {
    ...
    if (this->head_comp) {
        // promotekey = promotekey.substr(node->prefix.length());
        insertpos = insert_binary(node, newkey + node->prefix.length(),
                                  0, node->size - 1, equal);
    }
    else {
        insertpos = insert_binary(node, newkey, 0, node->size - 1, equal);
    ...
    if (this->head_comp) {
        InsertKey(node, insertpos, newkey + node->prefix.length());
    }
    else {
        InsertKey(node, insertpos, newkey);
    }
    ...
    if (this->head_comp) {
        newsplit.promotekey = node->prefix + firstright;
    }
    else {
    ...
    if (this->head_comp && pos >= 0) {
        ...
        string leftprefix = head_compression_find_prefix(lower_bound, newsplit.promotekey);
        string rightprefix = head_compression_find_prefix(newsplit.promotekey, upper_bound);

        left_mem = update_page_prefix(node, left_base, left_idx, leftprefix.length(), 0, split);
        right_mem = update_page_prefix(node, right_base, right_idx, rightprefix.length(), split + 1, node->size);

        node->prefix = leftprefix;
        right->prefix = rightprefix;
    }
    else {
        // split + 1: the two parent are like: |p-k-p|  |p-k-p-k-p|, no need to insert a k between them
        CopyKeyToPage(node, 0, split, left_base, left_mem, left_idx);
        CopyKeyToPage(node, split + 1, node->size, right_base, right_mem, right_idx);
    }
    ...
    copy(node->ptrs.begin() + split + 1, node->ptrs.end(), back_inserter(rightptrs));
...
splitReturn BPTree::split_leaf(Node *node, vector<Node *> &parents, char *newkey) {
    splitReturn newsplit;
    Node *right = new Node;
    int insertpos;
    bool equal = false;
    ...
    if (this->head_comp) {
        ...
        left_mem = update_page_prefix(node, left_base, left_idx, leftprefix.length(), 0, split);
        right_mem = update_page_prefix(node, right_base, right_idx, rightprefix.length(), split, node->size);
...
#ifdef TIME_DEBUG
splitReturn BPTree::split_leaf_time(Node *node, vector<Node *> &parents, char *newkey,
    ...
    if (this->head_comp) {
        ...
        get_node_bounds(parents, parents.size() - 1, node, lower_bound, upper_bound, lower_set, upper_set);
        ...
        prefix_calc += static_cast<double>(std::chrono::duration_cast<
                                               std::chrono::nanoseconds>(t_end - t_start)
                                               .count())
                       / 1e9;
        ...
        prefix_update += static_cast<double>(std::chrono::duration_cast<
                                                 std::chrono::nanoseconds>(t_end1 - t_start1)
                                                 .count())
                         / 1e9;
...
int BPTree::insert_binary(Node *cursor, const char *key, int low, int high, bool &equal) {
...
#else
        int cmp = char_cmp(key, GetKey(cursor, mid));
#endif
        if (cmp == 0) {
            equal = true;
            return mid + 1;
...
Node *BPTree::search_leaf_node(Node *searchroot, const char *key, vector<Node *> &parents) {
    // Tree is empty
    if (searchroot == NULL) {
    ...
    while (!cursor->IS_LEAF) {
        ...
        int pos;
        if (this->head_comp) {
            pos = insert_binary(cursor, key + cursor->prefix.length(),
                                0, cursor->size - 1, equal);
        }
        else {
            pos = insert_binary(cursor, key, 0, cursor->size - 1, equal);
        }
        // searchkey = key.substr(cursor->prefix.length());
        // pos = insert_binary(cursor, searchkey, 0, cursor->size - 1, equal);
        cursor = cursor->ptrs.at(pos);
    }
    return cursor;
}

int BPTree::search_binary(Node *cursor, const char *key, int low, int high) {
    while (low <= high) {
        int mid = low + (high - low) / 2;
#ifdef DUPKEY
        int cmp = char_cmp(key, cursor->keys.at(mid).value);
...
#ifdef DUPKEY
void BPTree::get_node_bounds(vector<Node *> parents, int pos,
                             Node *node, string &lower_bound, string &upper_bound) {
    Node *curr = node;
    bool lower_set = false;
    bool upper_set = false;
    for (int i = pos; i >= 0; i--) {
...
void BPTree::get_node_bounds(vector<Node *> parents, int pos,
    ...
    for (int i = pos; i >= 0; i--) {
        ...
        if (nodepos > 0 && nodepos < size) {
            if (!lower_set) {
                lower_bound = parent->prefix + GetKey(parent, nodepos - 1);
                lower_set = true;
            }
            if (!upper_set) {
                upper_bound = parent->prefix + GetKey(parent, nodepos);
                upper_set = true;
            }
        }
        else if (nodepos == 0) {
            if (!upper_set) {
                upper_bound = parent->prefix + GetKey(parent, 0);
                upper_set = true;
            }
        }
        else {
            if (!lower_set) {
                lower_bound = parent->prefix + GetKey(parent, size - 1);
                lower_set = true;
            }
        }
        curr = parent;
...
int BPTree::update_page_prefix(Node *node, char *page,
                               vector<uint16_t> &idx, int prefixlen, int low, int high) {
    // Compress node[low:high) to *page, return memusage
    uint16_t memusage = 0;
    // The new prefix must be longer, it's no need to use the original prefix
    int cutoff = prefixlen - node->prefix.length();
...
#endif
        totalKeySize += currSize + cursor->prefix.length();
        numKeys += cursor->size;
        totalPrefixSize += cursor->prefix.length();
        numNodes += 1;
        if (!cursor->IS_LEAF) {
...
void BPTree::printTree(Node *x, vector<bool> flag, bool compressed, int depth, bool isLast) {
