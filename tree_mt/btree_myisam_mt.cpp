...
int BPTreeMyISAMMT::backwardScan(string min, string max) {
    vector<NodeMyISAM *> parents;
...
vector<string> BPTreeMyISAMMT::decompress_keys(NodeMyISAM *node, int pos) {
...
int BPTreeMyISAMMT::prefix_insert(NodeMyISAM *cursor, string_view key, bool &equal){
    int cmplen;
    int len, matched;
    int my_flag;
...
int BPTreeMyISAMMT::insert_binary(NodeMyISAM *cursor, string_view key, int low, int high, bool &equal){
    while (low <= high){
...
NodeMyISAM* BPTreeMyISAMMT::scan_node(NodeMyISAM *node, string key){
    ...
    {
        bool equal = false;
        int pos = prefix_insert(node, key, equal);
