...
int search_prefix_metadata(DB2Node *cursor, string_view key){
    ...
    while (low <= high)
    {
        ...
        if (prefixcmp == 0)
        {
            ...
            int lowcmp = key.compare(prefixlen, len, cursor->keys.at(prefixlow));
            int highcmp = key.compare(prefixlen, len, cursor->keys.at(prefixhigh));
...
int find_insert_pos(DB2Node *node, string &key, int (*insertfunc)(DB2Node*, string_view, int, int)){
    ...
    if (metadatapos == node->prefixMetadata.size()){
        ...
        if (key.compare(0, prefix.length(), prefix) == 0){
            ...
            insertpos = insertfunc(node, key, metadata.low, metadata.high);
...
int calculate_prefix_merge_cost(vector<string> keys, vector<PrefixMetaData> segment, string prefix){
    ...
    for (uint32_t i = 0; i < segment.size(); i++){
        ...
        for (int j = low; j <= high; j++){
            string decompressed = segment.at(i).prefix + keys.at(j);
...
int calculate_cost_of_optimization(prefixOptimization result)
{
    ...
    for (int i = 0; i < result.prefixMetadatas.size(); i++)
    {
        ...
        for (int j = low; j <= high; j++)
        {
            cost += result.keys.at(j).length();
        }
    }
    return cost;
}

prefixMergeSegment find_best_segment_of_size_k(vector<string> keys, closedRange closedRange, int k){
...
prefixMergeSegment find_best_segment_in_closed_range(vector<string> keys, closedRange closedRange){
...
void merge_prefixes_in_segment(vector<string> &keys, vector<PrefixMetaData> &prefixmetadatas,
    ...
    for (uint32_t i = 0; i < segment.size(); i++){
        ...
        lasthigh = high;
        for (int j = low; j <= high; j++){
            string decompressed = prevprefix + keys.at(j);
            // Representing null prefix
            if (newprefix.compare("") == 0){
                keys.at(j) = decompressed;
            }else{
                keys.at(j) = decompressed.substr(newprefix.length());
            }
        }
    }
...
prefixOptimization prefix_merge(vector<string> keys, vector<PrefixMetaData> prefixmetadatas){
    ...
    while (nextindex < metadataCopy.size())
    {
        ...
        string p_i_first_key = p_i + keysCopy.at(metadataCopy.at(nextindex).low);
        string p_i_prev_last_key = p_i_minus_1 + keysCopy.at(metadataCopy.at(nextindex - 1).high);
...
int expand_prefixes_in_boundary(vector<string> &keys, vector<PrefixMetaData> &prefixmetadatas, int index){
    PrefixMetaData p_i = prefixmetadatas.at(index);
    PrefixMetaData p_i_plus_1 = prefixmetadatas.at(index + 1);
    string common_prefix = get_common_prefix(p_i.prefix + keys.at(p_i.high),
                                       p_i_plus_1.prefix + keys.at(p_i_plus_1.low));
    ...
    if (prefixlen > 0 && prefixlen >= max(p_i.prefix.length(), p_i_plus_1.prefix.length())){
        for (int i = p_i.low; i <= p_i.high; i++){
            string decompressed = p_i.prefix + keys.at(i);
            if (decompressed.compare(0, prefixlen, common_prefix) == 0){
                // First low has not been set
                if (firstlow == -1)
                    firstlow = i;
                keys.at(i) = decompressed.substr(prefixlen);
            }
        }

        for (int i = p_i_plus_1.low; i <= p_i_plus_1.high; i++){
            string decompressed = p_i_plus_1.prefix + keys.at(i);
            if (decompressed.compare(0, prefixlen, common_prefix) == 0){
                keys.at(i) = decompressed.substr(prefixlen);
                lasthigh = i;
            }else{
...
prefixOptimization prefix_expand(vector<string> keys, vector<PrefixMetaData> prefixmetadatas){
    int nextprefix = 0;
    prefixOptimization result;
    vector<string> keysCopy(keys);
    vector<PrefixMetaData> metadataCopy(prefixmetadatas);
