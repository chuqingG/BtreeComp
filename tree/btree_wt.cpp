...
void BPTreeWT::insert_nonleaf(NodeWT *node, NodeWT **path, int parentlevel, splitReturnWT *childsplit) {
    if (check_split_condition(node, childsplit->promotekey.size)) {
        ...
        // TODO: binary search cannot simply shift right,
        // but it's ok if that's not the last one
...
int BPTreeWT::search_insert_pos(NodeWT *cursor, const char *key, int keylen,
    ...
    while (low <= high) {
        ...
        if (cmp == 0) {
            equal = true;
            while (mid < high) {
                WThead *header = GetHeaderWT(cursor, mid + 1);
                if (header->key_len) // return the last one if keys are equal
                    break;
                mid++;
            }
            return mid + 1;
