...
#ifdef TIME_DEBUG
    std::vector<std::map<BenchmarkTypes, double>> pfx_calc_times(kIndexStructures.size());
    std::vector<std::map<BenchmarkTypes, double>> pfx_update_times(kIndexStructures.size());
#endif
...
void PerformanceBenchmarkResults(std::vector<map<BenchmarkTypes, vector<double>>> structure_benchmark_times) {
    for (auto benchmark : benchmarks) {
        ...
        std::ofstream myfile;
        if (write_to_file) {
            // string file_name = output_path + "/" + "performance_" + benchmarkStrMap.at(benchmark) + ".txt";
            string file_name = output_path + ".txt";
            myfile.open(file_name, fstream::out | ios::app);
            myfile << "\n"
...
void TreeStatisticBenchmarkResults(std::vector<vector<TreeStatistics>> structure_benchmark_statistics) {
    ...
    if (write_to_file) {
        ...
        myfile << "name,height,keysize,prefixsize,branchdegree,nodes,nonleafnodes\n";
    }
...
void RunBenchmark() {
    ...
    if (dataset == DatasetTypes::RANDOM) {
        ...
        generate_random_number(values_warmup, 100'000); // TODO(chuqing): now set a small value for test
#endif
    }
    else {
...
int main(int argc, char *argv[]) {
    ...
    if (output_file_arg != nullptr) {
        ...
        if (output_path_str == "auto") {
            output_path = generate_output_path(datasets_arg, key_numbers, iterations, 0);
        }
        else {
