...
private:
    ...
    int insert_binary(NodeWT *cursor, string_view key, int low, int high, uint8_t &skiplow, bool &equal);
    int search_binary(NodeWT *cursor, string_view key, int low, int high, uint8_t &skiplow);
    NodeWT *search_leaf_node(NodeWT *root, string_view key, vector<NodeWT *> &parents, uint8_t &skiplow);
    ...
    NodeWT *scan_node(NodeWT *node, string key, uint8_t &skiplow);
