...
char* string_conv(const char* key, int keylen);
...
inline void CopyToNewPageStd(Node *nptr, int low, int high, char *newbase, uint16_t cutoff, uint16_t &top) {//cutoff is potential head_comp ignored bytes
    for (int i = low; i < high; i++) {
        ...
        int key_len = oldhead->key_len;
            ...
            strncpy(presuf, oldhead->key_prefix, min(key_len, PV_SIZE));
...
#ifdef KN
            presuf = string_conv(presuf, oldhead->key_len); //convert forward
            if (cutoff != 0) presuf = string_conv(presuf + cutoff, oldhead->key_len - cutoff); //and back with cutoff
...
inline char* string_conv(const char* key, int keylen, int cutoff) {//unnormalized to normalized
    ...
    if (keylen <= PV_SIZE) { //keylen
        ...
        memeset(result, 0, PV_SIZE + 1);
        strncpy(key, result, PV_SIZE);
