...
int BPTreePkBMT::search(string_view key){
    ...
    int pos = -1;
    if (leaf != nullptr) {
        findNodeResult result = find_node(leaf, string(key), offset);
...
int BPTreePkBMT::searchRange(string min, string max){
    vector<NodePkB *> parents;
    int offset = 0;
    NodePkB *leaf = search_leaf_node(root, min, parents, offset);
    NodePkB *prevleaf = nullptr;
    int pos = -1;
    if (leaf != nullptr) {
        findNodeResult result = find_node(leaf, min, offset);
        if (result.low == result.high)
            pos = result.low;
    }
    int entries = 0;
    ...
    while (leaf != nullptr) {
        ...
        string leafkey = leaf->keys.at(pos).original;
...
void BPTreePkBMT::insert(string x){
    ...
    if (root->size == 0){
        root->keys.push_back(KeyPkB(0, x, it->second));
        root->IS_LEAF = true;
        root->size = 1;
        root->unlock(WRITE);
        return;
...
void BPTreePkBMT::insert_nonleaf(NodePkB *node, vector<NodePkB *> &parents, int pos, splitReturnPkB childsplit){
    ...
    if (check_split_condition(node)){
        ...
        if (node == root){
            ...
            newRoot->keys.push_back(KeyPkB(0, currsplit.promotekey, currsplit.keyptr));
        ...
        int insertpos;
        findNodeResult result = find_node(node, promotekey, offset);
        insertpos = result.high;
        vector<KeyPkB> allkeys;
        for (int i = 0; i < insertpos; i++){
            allkeys.push_back(node->keys.at(i));
        }
        KeyPkB pkbKey = generate_pkb_key(node, promotekey, childsplit.keyptr, insertpos, parents, pos);
...
void BPTreePkBMT::insert_leaf(NodePkB *leaf, vector<NodePkB *> &parents, string key, char *keyptr, int offset){
    ...
    if (check_split_condition(leaf)){
        ...
        if (leaf == root){
            NodePkB *newRoot = new NodePkB;
            newRoot->keys.push_back(KeyPkB(0, split.promotekey, split.keyptr));
        ...
        int insertpos;
        findNodeResult result = find_node(leaf, key, offset);
        insertpos = result.high;
        ...
        KeyPkB pkbKey = generate_pkb_key(leaf, key, keyptr, insertpos, parents, parents.size() - 1);
        allkeys.push_back(pkbKey);
...
uncompressedKey BPTreePkBMT::get_uncompressed_key_before_insert(NodePkB *node, int ind, int insertpos, string newkey, char *newkeyptr){
...
splitReturnPkB BPTreePkBMT::split_nonleaf(NodePkB *node, vector<NodePkB *> parents, int pos, splitReturnPkB childsplit, int offset){
    ...
    KeyPkB pkbKey = generate_pkb_key(node, promotekey, promotekeyptr, insertpos, parents, pos);
    allkeys.push_back(pkbKey);
    ...
    uncompressedKey firstrightKeyAndPtr = get_uncompressed_key_before_insert(node, split + 1, insertpos, promotekey, promotekeyptr);
    uncompressedKey splitKeyAndPtr = get_uncompressed_key_before_insert(node, split, insertpos, promotekey, promotekeyptr);
    ...
    rightkeys.at(0) = KeyPkB(firstrightoffset, firstrightKeyAndPtr.key, firstrightKeyAndPtr.keyptr);
...
splitReturnPkB BPTreePkBMT::split_leaf(NodePkB *node, vector<NodePkB *> &parents, string newkey, char *newkeyptr, int offset){
    ...
    findNodeResult result = find_node(node, newkey, offset);
    insertpos = result.high;
    ...
    KeyPkB pkbKey = generate_pkb_key(node, newkey, newkeyptr, insertpos, parents, parents.size() - 1);
    allkeys.push_back(pkbKey);
    ...
    string firstright;
    uncompressedKey firstrightKeyAndPtr = get_uncompressed_key_before_insert(node, split, insertpos, newkey, newkeyptr);
    firstright = firstrightKeyAndPtr.key;
    rightkeys.at(0) = KeyPkB(firstright.length(), firstright, firstrightKeyAndPtr.keyptr);
...
NodePkB* BPTreePkBMT::scan_node(NodePkB *node, string key, int &offset){
    ...
    {
        int pos;
        findNodeResult result = find_node(node, key, offset);
