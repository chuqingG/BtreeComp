...
// in B+ Tree
int BPTreeDB2MT::search(string_view key){
    vector<DB2Node *> parents;
    DB2Node *leaf = search_leaf_node(root, key, parents);
    int pos;
    if (leaf == nullptr){
        pos = -1;
    }
...
int BPTreeDB2MT::searchRange(string min, string max){
    ...
    while (pos != -1 && leaf != nullptr) {
        ...
        string leafkey = leaf->prefixMetadata.at(metadatapos).prefix + leaf->keys.at(pos);
...
void BPTreeDB2MT::insert_nonleaf(DB2Node *node, vector<DB2Node *> &parents, int pos, splitReturnDB2 childsplit){
    ...
    if (check_split_condition(node)){
        ...
        int insertpos = find_insert_pos(node, promotekey, this->insert_binary);
...
void BPTreeDB2MT::insert_leaf(DB2Node *leaf, vector<DB2Node *> &parents, string key){
    ...
    if (check_split_condition(leaf)){
        ...
        int insertpos;
        insertpos = find_insert_pos(leaf, key, this->insert_binary);
...
int BPTreeDB2MT::split_point(vector<string> allkeys){
...
splitReturnDB2 BPTreeDB2MT::split_nonleaf(DB2Node *node, vector<DB2Node *> parents, int pos, splitReturnDB2 childsplit){
    ...
    string promotekey = childsplit.promotekey;
    int insertpos;
    insertpos = find_insert_pos(node, promotekey, this->insert_binary);
    vector<string> allkeys;
    vector<DB2Node *> allptrs;
    ...
    newsplit.promotekey = splitresult.splitprefix + allkeys.at(split);
...
splitReturnDB2 BPTreeDB2MT::split_leaf(DB2Node *node, vector<DB2Node *> &parents, string newkey){
    ...
    DB2Node *right = new DB2Node;
    int insertpos;
    insertpos = find_insert_pos(node, newkey, this->insert_binary);
...
int BPTreeDB2MT::insert_binary(DB2Node *cursor, string_view key, int low, int high){
    while (low <= high){
...
DB2Node* BPTreeDB2MT::search_leaf_node(DB2Node *root, string_view key, vector<DB2Node *> &parents)
{
    // Tree is empty
    if (root->size == 0){
    ...
    while (!cursor->IS_LEAF) {
        ...
        DB2Node *nextPtr = scan_node(cursor, string(searchkey));
...
int BPTreeDB2MT::search_binary(DB2Node *cursor, string_view key, int low, int high){
    ...
    cout << "key:" << key << endl;
    cout << "node: " << cursor->keys.at(0) << endl;
    return -1;
}

DB2Node* BPTreeDB2MT::scan_node(DB2Node *node, string key){
    ...
    else
    {
        ...
        if (metadatapos == node->prefixMetadata.size()){
            ...
            if (prefixcmp != 0){
                ...
                pos = insert_binary(node, searchkey, currentMetadata.low, currentMetadata.high);
            }
        }
