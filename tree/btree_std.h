...
private:
    ...
    Node *search_leaf_node(Node *root, const char *key, vector<Node *> &parents);
#ifdef DUPKEY
    ...
    void get_node_bounds(vector<Node *> parents, int pos,
                         Node *node, string &lower_bound, string &upper_bound);
    vector<string> decompress_keys(Node *node, int pos = -1);
    int update_page_prefix(Node *node, char *page, vector<uint16_t> &idx, int prefixlen, int low, int high);
