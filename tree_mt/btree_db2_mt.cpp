...
// Initialise the BPTreeDB2MT NodeDB2
...
int BPTreeDB2MT::search(string_view key, bool print) {
    vector<NodeDB2 *> parents;
    NodeDB2 *leaf = search_leaf_node(root, key, parents, print);
    int pos;
    if (leaf == nullptr) {
        pos = -1;
    }
...
void BPTreeDB2MT::insert(string x) {
    ...
    vector<NodeDB2 *> parents;
    NodeDB2 *leaf = search_leaf_node(root, x, parents);
    insert_leaf(leaf, parents, x);
}

void BPTreeDB2MT::insert_nonleaf(NodeDB2 *node, vector<NodeDB2 *> &parents, int pos, splitReturnDB2 childsplit) {
    node->lock(WRITE);
...
void BPTreeDB2MT::insert_leaf(NodeDB2 *leaf, vector<NodeDB2 *> &parents, string key) {
    ...
    if (check_split_condition(leaf)) {
        ...
        if (leaf == root) {
            ...
            NodeDB2 *parent = parents.at(parents.size() - 1);
...
splitReturnDB2 BPTreeDB2MT::split_nonleaf(NodeDB2 *node, vector<NodeDB2 *> parents, int pos, splitReturnDB2 childsplit) {
...
splitReturnDB2 BPTreeDB2MT::split_leaf(NodeDB2 *node, vector<NodeDB2 *> &parents, string newkey) {
    splitReturnDB2 newsplit;
    NodeDB2 *right = new NodeDB2;
    int insertpos;
    int rid = rand();
...
bool BPTreeDB2MT::check_split_condition(NodeDB2 *node) {
    int currspace = 0;
...
int BPTreeDB2MT::insert_binary(NodeDB2 *cursor, string_view key, int low, int high, bool &equal) {
    while (low <= high) {
...
NodeDB2 *BPTreeDB2MT::search_leaf_node(NodeDB2 *root, string_view key, vector<NodeDB2 *> &parents, bool print) {
    if (print)
    ...
    while (!cursor->IS_LEAF) {
        ...
        NodeDB2 *nextPtr = scan_node(cursor, string(searchkey), print);
...
int BPTreeDB2MT::search_binary(NodeDB2 *cursor, string_view key, int low, int high) {
    while (low <= high) {
        int mid = low + (high - low) / 2;
        int cmp = lex_compare(string(key), cursor->keys.at(mid).value);
        if (cmp == 0)
...
NodeDB2 *BPTreeDB2MT::scan_node(NodeDB2 *node, string key, bool print) {
...
NodeDB2 *BPTreeDB2MT::move_right(NodeDB2 *node, string key) {
    NodeDB2 *current = node;
    NodeDB2 *nextPtr;
...
void BPTreeDB2MT::bt_fetch_root(NodeDB2 *currRoot, vector<NodeDB2 *> &parents) {
    NodeDB2 *cursor = root;
    ...
    while (cursor->level != currRoot->level) {
        NodeDB2 *nextPtr = scan_node(cursor, key);
        if (nextPtr != cursor->next) {
            parents.push_back(cursor);
        }
...
void BPTreeDB2MT::getSize(NodeDB2 *cursor, int &numNodes, int &numNonLeaf, int &numKeys,
...
int BPTreeDB2MT::getHeight(NodeDB2 *cursor) {
    if (cursor == NULL)
        return 0;
    if (cursor->IS_LEAF == true)
        return 1;
...
void BPTreeDB2MT::printTree(NodeDB2 *x, vector<bool> flag, bool compressed, int depth, bool isLast) {
