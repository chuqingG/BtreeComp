...
BPTree::~BPTree() { delete _root; }

// Function to get the rootNode
Node *BPTree::getRoot() { return _root; }

// Function to find any element
// in B+ Tree
int BPTree::search(const char *key)
{
    Node *leaf = search_leaf_node(_root, key);
    if (leaf == nullptr)
        return -1;
    if (this->head_comp)
    {
        return search_in_leaf(leaf, key + leaf->prefix.size(), 0, leaf->size - 1);
    }
    else
    {
...
void BPTree::insert(char *x)
{
    ...
    Node *leaf = search_leaf_node_for_insert(_root, x, parents);
...
vector<string> BPTree::decompress_keys(Node *node, int pos)
{
    ...
    {
        string key = head_comp ? node->prefix + node->keys.at(i).value
                               : node->keys.at(i).value;
...
void BPTree::insert_nonleaf(Node *node, vector<Node *> &parents,
                            int parentlevel, splitReturn childsplit)
{
...
splitReturn BPTree::split_nonleaf(Node *node, vector<Node *> parents, int pos,
                                  splitReturn childsplit)
{
    ...
    if (this->head_comp && pos >= 0)
    {
        ...
        string leftprefix =
            head_compression_find_prefix(lower_bound, newsplit.promotekey);
        string rightprefix =
            head_compression_find_prefix(newsplit.promotekey, upper_bound);
...
splitReturn BPTree::split_leaf(Node *node, vector<Node *> &parents,
                               char *newkey)
{
    splitReturn newsplit;
    Node *right = new Node;
...
int BPTree::search_insert_pos(Node *cursor, const char *key, int low, int high,
                              bool &equal)
{
...
Node *BPTree::search_leaf_node(Node *searchroot, const char *key)
{
    // Tree is empty
    if (searchroot == NULL)
    {
    ...
    while (!cursor->IS_LEAF)
    {
        ...
        if (this->head_comp)
        {
            pos = search_in_nonleaf(cursor, key + cursor->prefix.length(), 0,
                                    cursor->size - 1);
        }
        else
        {
            pos = search_in_nonleaf(cursor, key, 0, cursor->size - 1);
        }
...
Node *BPTree::search_leaf_node_for_insert(Node *searchroot, const char *key,
                                          vector<Node *> &parents)
{
    // Tree is empty
    if (searchroot == NULL)
    ...
    while (!cursor->IS_LEAF)
    {
        ...
        if (this->head_comp)
        {
            pos = search_insert_pos(cursor, key + cursor->prefix.length(), 0,
                                    cursor->size - 1, equal);
        }
        else
        {
            pos = search_insert_pos(cursor, key, 0, cursor->size - 1, equal);
        }
...
int BPTree::search_in_nonleaf(Node *cursor, const char *key, int low,
                              int high)
{
    // char* page = cursor->base;
    while (low <= high)
...
int BPTree::search_in_leaf(Node *cursor, const char *key, int low, int high)
{
    while (low <= high)
    {
        int mid = low + (high - low) / 2;
