...
void BPTreeWTMT::insert_nonleaf(NodeWT *node, vector<NodeWT *> &parents, int pos, splitReturnWT childsplit){
    ...
    if (check_split_condition(node)){
        ...
        if (node == root){
            ...
            newRoot->keys.push_back(KeyWT(currsplit.promotekey, 0));
        ...
        size_t skiplow = 0;
        int insertpos = insert_binary(node, promotekey, 0, node->size - 1, skiplow);
        ...
        if(this->non_leaf_comp){
            if (insertpos > 0){
                ...
                allkeys.push_back(KeyWT(compressed, pfx));
            }else{
                allkeys.push_back(KeyWT(promotekey, 0));
            }
        } else {
            allkeys.push_back(KeyWT(promotekey, 0));
        }
...
void BPTreeWTMT::insert_leaf(NodeWT *leaf, vector<NodeWT *> &parents, string key){
    ...
    if (check_split_condition(leaf)){
        ...
        if (leaf == root){
            NodeWT *newRoot = new NodeWT;
            newRoot->keys.push_back(KeyWT(split.promotekey, 0));
        ...
        size_t skiplow = 0;
        int insertpos = insert_binary(leaf, key, 0, leaf->size - 1, skiplow);
        vector<KeyWT> allkeys;
...
splitReturnWT BPTreeWTMT::split_nonleaf(NodeWT *node, vector<NodeWT *> parents, int pos, splitReturnWT childsplit){
    ...
    for (int i = 0; i < insertpos; i++){
        allkeys.push_back(KeyWT(node->keys.at(i).value, node->keys.at(i).prefix));
    }
    ...
    if(this->non_leaf_comp){
        ...
        rightkeys.at(0) = KeyWT(firstright, 0);
...
splitReturnWT BPTreeWTMT::split_leaf(NodeWT *node, vector<NodeWT *> &parents, string newkey){
    ...
    NodeWT *right = new NodeWT;
    size_t skiplow = 0;
    int insertpos = insert_binary(node, newkey, 0, node->size - 1, skiplow);
    vector<KeyWT> allkeys;
    ...
    if (insertpos > 0){
        ...
        allkeys.push_back(KeyWT(newkey, 0));
    }
    ...
    if(this->suffix_comp){
        string lastleft = get_uncompressed_key_before_insert(node, split - 1, insertpos, newkey);
...
int BPTreeWTMT::insert_binary(NodeWT *cursor, string_view key, int low, int high, size_t &skiplow){
    stringstream ss;
    size_t skiphigh = 0;
    while (low <= high){
...
NodeWT* BPTreeWTMT::scan_node(NodeWT *node, string key, size_t &skiplow){
    ...
    {
        int pos = insert_binary(node, key, 0, node->size - 1, skiplow);
