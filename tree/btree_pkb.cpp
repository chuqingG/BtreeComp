...
// Function to peform range query on B+Tree of PKB
int BPTreePkB::searchRange(string min, string max){
    vector<NodePkB *> parents;
    int offset = 0;
    ...
    bool equal = false;
    if (leaf != nullptr) {
        findNodeResult result = find_node(leaf, min, offset, equal);
        if (result.low == result.high)
            pos = result.low;
    }
    int entries = 0;
...
int BPTreePkB::backwardScan(string min, string max) {
    vector<NodePkB *> parents;
    int offset = 0;
    NodePkB *leaf = search_leaf_node(root, max, parents, offset);
    int pos = -1;
    bool equal = false;
    if (leaf != nullptr) {
        findNodeResult result = find_node(leaf, max, offset, equal);
        if (result.low == result.high)
            pos = result.low;
    }
    int entries = 0;
...
vector<string> BPTreePkB::decompress_keys(NodePkB *node, int pos) {
    vector<string> decompressed_keys;
    if (node == nullptr)
        return decompressed_keys;
    int size = pos == -1 ? node->keys.size() - 1 : pos; 
    for(int i = 0; i <= size; i++) {
        string key = node->keys.at(i).original;
